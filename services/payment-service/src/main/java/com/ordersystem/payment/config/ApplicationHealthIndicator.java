...
/**
 * Comprehensive application health indicator for Payment Service
 */
    ...
    public Health health() {
        try {
            ...
            if (isHealthy) {
                ...
                details.put("message", "Payment Service is fully operational");
                logger.debug("Payment Service health check: HEALTHY");
                return Health.up().withDetails(details).build();
            } else {
                details.put("status", "UNHEALTHY");
                details.put("message", "One or more dependencies are not available");
                logger.warn("Payment Service health check: UNHEALTHY - RabbitMQ: {}, Startup: {}", 
            ...
            logger.error("Payment Service application health check failed", e);
            return Health.down()
