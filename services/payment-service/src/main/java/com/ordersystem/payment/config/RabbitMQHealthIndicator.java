...
/**
 * Custom health indicator for RabbitMQ connectivity in Payment Service
 */
@Component("rabbitmq")
public class RabbitMQHealthIndicator implements HealthIndicator {
    ...
    public Health health() {
        try {
            ...
            logger.error("Payment Service RabbitMQ health check failed", e);
            return Health.down()
                    .withDetail("error", e.getMessage())
                    .withDetail("timestamp", Instant.now())
                    .withDetail("status", "Payment Service RabbitMQ connection failed")
                    .build();
        }
    }
    ...
    private Health checkRabbitMQHealth() throws Exception {
        ...
        try (com.rabbitmq.client.Connection connection = connectionFactory.createConnection().getDelegate()) {
            ...
            logger.debug("Payment Service RabbitMQ health check successful in {}ms", responseTime);
            
            return Health.up().withDetails(details).build();
            
        } catch (Exception e) {
            logger.warn("Payment Service RabbitMQ health check failed, will retry: {}", e.getMessage());
            throw e;
        }
    }
}