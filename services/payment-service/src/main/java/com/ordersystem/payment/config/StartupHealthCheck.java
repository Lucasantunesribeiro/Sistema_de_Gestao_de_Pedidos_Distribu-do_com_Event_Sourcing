...
/**
 * Startup coordination and health verification for Payment Service
 */
@Component
public class StartupHealthCheck {
    ...
    public void onApplicationReady() {
        logger.info("Payment Service application ready event received, starting dependency verification...");
        ...
        try {
            ...
            logger.info("Payment Service startup completed successfully in {}ms", startupDuration);
            logger.info("Payment Service is ready to process payments");
            
            logConfigurationSummary();
            
        } catch (Exception e) {
            logger.error("Payment Service startup failed during dependency verification", e);
            isReady = false;
            throw new RuntimeException("Payment Service startup failed", e);
        }
    }

    @Retryable(value = {Exception.class}, maxAttempts = 5, backoff = @Backoff(delay = 2000, multiplier = 2))
    private void verifyAllDependencies() throws Exception {
        logger.info("Verifying Payment Service dependencies...");
        
        // Verify RabbitMQ connectivity
        verifyRabbitMQConnection();
        
        logger.info("All Payment Service dependencies verified successfully");
    }
    ...
    private void logConfigurationSummary() {
        logger.info("=== Payment Service Configuration Summary ===");
        logger.info("Service: Payment Service");
        logger.info("Port: 8082");
