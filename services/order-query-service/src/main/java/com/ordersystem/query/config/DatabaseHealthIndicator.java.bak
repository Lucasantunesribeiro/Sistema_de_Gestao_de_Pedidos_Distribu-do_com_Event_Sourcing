...
/**
 * Custom health indicator for query database connectivity
 * Provides detailed health information about read database connection status
 */
...
public class DatabaseHealthIndicator implements HealthIndicator {
    ...
    private static final String HEALTH_CHECK_QUERY = "SELECT 1 as health_check, current_timestamp as check_time, version() as db_version";
    ...
    public Health health() {
        try {
            ...
            logger.error("Query database health check failed", e);
            return Health.down()
                    .withDetail("error", e.getMessage())
                    .withDetail("timestamp", Instant.now())
                    .withDetail("status", "Query database connection failed")
                    .build();
        }
    }
    ...
    private Health checkDatabaseHealth() throws SQLException {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            if (resultSet.next()) {
                ...
                details.put("database", "PostgreSQL (Query DB)");
                ...
                details.put("dbVersion", resultSet.getString("db_version"));
                ...
                if (dataSource instanceof com.zaxxer.hikari.HikariDataSource) {
                    ...
                    if (poolBean != null) {
                        ...
                        details.put("pool.maxPoolSize", hikariDataSource.getMaximumPoolSize());
                        details.put("pool.minIdle", hikariDataSource.getMinimumIdle());
                    }
                }
                
                logger.debug("Query database health check successful in {}ms", responseTime);
            ...
            logger.warn("Query database health check failed, will retry: {}", e.getMessage());
            throw e;
        }
    }
